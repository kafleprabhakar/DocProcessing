...
def sort_horizontal_contours(contours):
    ...
    while i < len(all_coord):
        ...
        #print(i)
        label_seen=False
        for contour in all_coord_final: #this part is added, logic is we don't want to start a new label that appears in the same horizontal span where an upper label has appeared
            if contour[0]<=all_coord[i][0]<=contour[0]+contour[2]:
                label_seen=True
                break
        if label_seen:
            i+=1
...
def check_table_type(path: str = None , jsonFile=None):

    img = cv2.imread(path, 0) #including the 0 means it turns into a grayscale image

    im_color = cv2.imread(path)
    im_color_line = cv2.imread(path)
    imgcopy= cv2.imread(path)

    thresh, img_bin = cv2.threshold(img, 128, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU) #threshold doesn't seem to be doing anything?
    #cv2.waitKey(0)
    img_bin = 255 - img_bin
    #added
    ...
    image_ver = cv2.erode(img_bin, ver_kernel, iterations=2)
    ...
    cv2.imshow('horizontallines', im_color_line)
    cv2.waitKey(0)
    ...
    for cluster in cluster_lines:
        ...
        for i in range(len(cluster)-1):
            ...
            if i==0:
                ...
                if len(standard)!=len(verlines):
                    not_aligned=True
                else:
                    for k in range(len(standard)):
                        if abs(standard[k][0]-verlines[k][0])>5:
                            not_aligned=True
                            break
                            
            if not_aligned:
                top_left = (cluster[0][0],cluster[0][1])
                irregular_shaped.append(top_left)
                cv2.rectangle(imgcopy, top_left, (cluster[-1][2], cluster[-1][3]), (0, 255, 0), 3)
                break
    dims = img.shape
    imgcopy = cv2.resize(imgcopy,(int(dims[1] / 3), int(dims[0] / 3)))
    cv2.imshow('nonuniform',imgcopy)

    cv2.waitKey(0)

    return irregular_shaped
...
def get_table_segments(image: np.ndarray, debug: bool = False) -> Tuple[np.ndarray, List[Box]]:
    ...
    imvh = cv2.resize(im_vh_color, (dims[0]//3, dims[1]//3))
    cv2.imshow('the segments is', imvh)
    cv2.waitKey(0)
...
def extract_tables(path, outfile: str = None, debug: bool = False) -> List[List[List[str]]]:
    ...
    tables = []
    compare= []
    irregular_shaped = check_table_type(path)
    print ('irregular shaped is', irregular_shaped)
    tops=[]
    for table in table_boxes:
        top_left, _= table.get_box_endpoints()
        tops.append(top_left)
        is_irregular_shaped=False
        for x,y in irregular_shaped:
            if math.sqrt((top_left[0]-x)**2+(top_left[1]-y)**2)<40:
                is_irregular_shaped=True
                break
        if is_irregular_shaped:
            continue
